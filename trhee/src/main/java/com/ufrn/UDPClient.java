...
public class UDPClient {
    public static void main(String[] args) throws Exception {
        // Validação dos argumentos (IP e Porta do servidor)
        if (args.length < 2) {
        ...
        // Timeout de 2 segundos para o programa não travar se um pacote sumir [cite: 182]
        udpclient.setSoTimeout(2000);

        // Tamanhos exigidos na tabela do slide 40
        ...
        for (int kb : tamanhosKB) {
            ...
            byte[] dados = new byte[tamanhoBytes]; // Gera o "peso" do pacote
            ...
            try {
                ...
                // Envia o pacote de dados [cite: 27]
                udpclient.send(pedido);

                // Espera o ACK (Confirmação) do Servidor [cite: 371]
                ...
                // Cálculo da taxa de transmissão em Mbps
                // (Bytes * 8 bits) / (Segundos * 1.000.000)
                double mbps = (tamanhoBytes * 8.0) / (tempoMs / 1000.0) / 1_000_000.0;

                System.out.printf("%d KB\t\t%.3f ms\t\t%.3f Mbps\n", kb, tempoMs, mbps);

            } catch (java.net.SocketTimeoutException e) {
                System.out.println(kb + " KB\t\tFALHA (Pacote perdido)");
            }
        }
